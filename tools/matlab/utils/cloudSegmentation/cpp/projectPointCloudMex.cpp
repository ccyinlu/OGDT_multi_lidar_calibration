    ...
    for(int i = 0; i < numOfPoints; i++){
        ...
        if(nlhs > 2){
            ...
            }else if (v_angle < vertical_theta[N_SCAN - 1])
            {
                ...
                if(vertical_theta[0] - vertical_theta[N_SCAN - 1] == 0){
                    v_index_raw = 0;
                }else{
        ...
        #ifdef DEBUG
            mexPrintf("%s%f\t%s%f\t%s%f\t%s%f\t%s%f\t%s%d\t%s%d\t%s%d\n", "x: ", current_x, "y: ", current_y, "z: ", current_z, "h_angle: ", h_angle, "v_angle: ", v_angle, "v_index_raw: ", v_index_raw, "h: ", h_index, "v: ", v_index);
        #endif
