...
enables = {
    true, ... % #1 lidar_front pointcloud map generation from relative trajectory
    true, ... % #2 lidar_left pointcloud map generation from relative trajectory
    true, ... % #3 lidar_right pointcloud map generation from relative trajectory
    true, ... % #4 lidar_back pointcloud map generation from relative trajectory
}; 
...
%% #1
% lidar_front pointcloud map generation from relative trajectory
if enables{1}
    fprintf('########################### #1 lidar_front pointcloud map generation from relative trajectory   ##########################\n');
    for i = start_index : lidar_front_observation_num
        % read the pointcloud
        current_pc = pcread(sprintf('%s/%06d.pcd', lidar_front_data_dir, i));
        current_pose_relative = lidar_front_trajectory_relative(i, :);
        current_transform_matrix_relative = transform_matrix_from_pose([
            current_pose_relative(4) ...
            current_pose_relative(5) ...
            current_pose_relative(6) ...
            current_pose_relative(1) ...
            current_pose_relative(2) ...
            current_pose_relative(3) ...
        ]);
        if i == start_index
            acc_transform_matrix_relative = current_transform_matrix_relative;
        end
        acc_transform_matrix_relative = acc_transform_matrix_relative * current_transform_matrix_relative;

        current_pc_transformed = pctransform(current_pc, affine3d(acc_transform_matrix_relative'));
        if i == start_index
    ...
    pcwrite(lidar_front_pc_map, sprintf('%s/%s', data_root, lidar_front_map_relative));
end

%% #2
% lidar_left pointcloud map generation from relative trajectory
if enables{2}
    fprintf('########################### #2 lidar_left pointcloud map generation from relative trajectory   ##########################\n');
    for i = start_index : lidar_left_observation_num
        % read the pointcloud
        current_pc = pcread(sprintf('%s/%06d.pcd', lidar_left_data_dir, i));
        current_pose_relative = lidar_left_trajectory_relative(i, :);
        current_transform_matrix_relative = transform_matrix_from_pose([
            current_pose_relative(4) ...
            current_pose_relative(5) ...
            current_pose_relative(6) ...
    ...
    pcwrite(lidar_left_pc_map, sprintf('%s/%s', data_root, lidar_left_map_relative));
end

%% #3
% lidar_right pointcloud map generation from relative trajectory
if enables{3}
    fprintf('########################### #3 lidar_right pointcloud map generation from relative trajectory   ##########################\n');
    for i = start_index : lidar_right_observation_num
        % read the pointcloud
        current_pc = pcread(sprintf('%s/%06d.pcd', lidar_right_data_dir, i));
        current_pose_relative = lidar_right_trajectory_relative(i, :);
        current_transform_matrix_relative = transform_matrix_from_pose([
            current_pose_relative(4) ...
            current_pose_relative(5) ...
            current_pose_relative(6) ...
    ...
    pcwrite(lidar_right_pc_map, sprintf('%s/%s', data_root, lidar_right_map_relative));
end

%% #4
% lidar_back pointcloud map generation from relative trajectory
if enables{4}
    fprintf('########################### #4 lidar_back pointcloud map generation from relative trajectory   ##########################\n');
    for i = start_index : lidar_back_observation_num
        % read the pointcloud
        current_pc = pcread(sprintf('%s/%06d.pcd', lidar_back_data_dir, i));
        current_pose_relative = lidar_back_trajectory_relative(i, :);
        current_transform_matrix_relative = transform_matrix_from_pose([
            current_pose_relative(4) ...
            current_pose_relative(5) ...
            current_pose_relative(6) ...
    ...
    pcwrite(lidar_back_pc_map, sprintf('%s/%s', data_root, lidar_back_map_relative));
end